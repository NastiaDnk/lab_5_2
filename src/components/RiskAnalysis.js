...
export const RiskAnalysis = () => {
    ...
    var ER = new Array(46);
    var LRER = new Array(46);
    var VRER = new Array(46);
    ...
    for(var i = 0; i < ER.length; ++i) 
        ER[i] = <td id={`ER-${i}`}>{Math.round(0.1 * list[i].reduce((partialSum, a) => partialSum + a.props.children.props.defaultValue, 0) * 1000) / 1000}</td>;

    for(var i = 0; i < LRER.length; ++i)
        LRER[i] = <td id={`LRER-${i}`}>{Math.round(Math.random() * 1000) / 1000}</td>;

    for(var i = 0; i < VRER.length; ++i)
        VRER[i] = <td id={`VRER-${i}`}>{Math.round(Math.random() * 1000 * LRER[i].props.children) / 1000}</td>;
    
    min = Math.min(...VRER.map(function(vrer) {
        return vrer.props.children;
    }));

    max = Math.max(...VRER.map(function(vrer) {
        return vrer.props.children;
    }));
    ...
    {
        var res = 'низький';
        if(VRER[i].props.children >= min && VRER[i].props.children < min + mpr)
            res = 'низький';
        if(VRER[i].props.children >= min + mpr && VRER[i].props.children < min + 2 * mpr)
            res = 'середній';
        if(VRER[i].props.children >= min + 2 * mpr & VRER[i].props.children <= max)
            res = 'високий';
        Priority[i] = <td id={`priority-${i}`}>{res}</td>;
    }

    var erSUM = ER.reduce((partialSum, a) => partialSum + a.props.children, 0);
    var eri = new Array(4).fill(0);
    ...
    for(var i = 0; i < eri.length; ++i) 
    {
        ...
        for(var j = from; j < to; ++j)
            eri[i] += ER[j].props.children;
        risks[i] = Math.round(1 / erSUM * eri[i] * 1000 * 100) / 1000;
    }
    ...
    const recalculateValues = () => {
        for(var i = 0; i < ER.length; ++i) {
            ...
            document.getElementById(`ER-${i}`).innerHTML = Math.round(0.1 * sum * 1000) / 1000;
        }
 
        for(var i = 0; i < VRER.length; ++i) {
            document.getElementById(`VRER-${i}`).innerHTML = Math.round(Math.random() * 1000 * document.getElementById(`LRER-${i}`).innerHTML) / 1000;
        }

        min = Math.min(...Array.from({length : VRER.length}).map((_, i) => {
            return Number(document.getElementById(`VRER-${i}`).innerHTML);
        }));

        max = Math.max(...Array.from({length : VRER.length}).map((_, i) => {
            return Number(document.getElementById(`VRER-${i}`).innerHTML);
        }));
        ...
        {
            let res = 'низький';
            if(Number(document.getElementById(`VRER-${i}`).innerHTML) >= min && Number(document.getElementById(`VRER-${i}`).innerHTML) < min + mpr)
                res = 'низький';
            if(Number(document.getElementById(`VRER-${i}`).innerHTML) >= min + mpr && Number(document.getElementById(`VRER-${i}`).innerHTML) < min + 2 * mpr)
                res = 'середній';
            if(Number(document.getElementById(`VRER-${i}`).innerHTML) >= min + 2 * mpr & Number(document.getElementById(`VRER-${i}`).innerHTML) <= max)
                res = 'високий';
        ...
        erSUM = 0;
        for(var i = 0; i < ER.length; ++i)
            erSUM += Number(document.getElementById(`ER-${i}`).innerHTML);
        eri = new Array(4).fill(0);
        risks = new Array(4).fill(0);
        ...
        for(var i = 0; i < eri.length; ++i) 
        {
            ...
            for(var j = from; j < to; ++j)
                eri[i] += Number(document.getElementById(`ER-${j}`).innerHTML);
            risks[i] = Math.round(1 / erSUM * eri[i] * 1000 * 100) / 1000;
        }
    ...
    return(
        <div>
            <h1 className='text-danger'>Аналіз ризиків</h1>
            ...
            <Modal.Dialog className="mt-4 mb-2">
                ...
                <Modal.Body className='result-font'>
